...
if ($_SERVER['REQUEST_METHOD'] == 'PUT') {
    ...
    try {
        $updatable_fields = ['username', 'email', 'phone', 'full_name', 'role', 'commission_rate', 'is_active'];
        $update_data = [];
        $update_sql = [];
        ...
        foreach ($updatable_fields as $field) {
            if (isset($input[$field])) {
                ...
                if ($field == 'username' || $field == 'email') {
                    $check_stmt = $pdo->prepare("SELECT id FROM users WHERE $field = ? AND id != ?");
        ...
        if (!empty($input['password'])) {
            $update_sql[] = "password = ?";
            $update_data[] = $input['password']; // في الإصدار النهائي هن encrypt
        }
