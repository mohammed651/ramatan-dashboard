...
export default function AddProjectModal({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // إعادة جلب المشاريع أو تحديث محلي (نسخة مبسطة: fetch again)
      dispatch(fetchProjects());
      onClose?.();
    } catch (err) {
      setError(err || "فشل إضافة المشروع");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 ">
      ...
      <form className="relative bg-white rounded shadow w-full max-w-lg p-6 z-10" onSubmit={handleSubmit}>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button type="submit" disabled={loading} className="px-4 py-1 bg-blue-600  rounded">
